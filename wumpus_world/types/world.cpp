...
void world::grid_init()
{
	...
	pair<int, int> resticted_position2; //to hold the restricted position adjcent to the agent
	agent_room_id = make_pair(1, 1);
	restricted_position = make_pair(1, 2);
	resticted_position2 = make_pair(2, 1);

	room agent_initial_room(agent_room_id);
	room restricted_room(restricted_position);
	room restricted_room2(resticted_position2);

	this->grid[1][1] = agent_initial_room;
	this->grid[1][2] = restricted_room;
	this->grid[2][1] = restricted_room2;
	...
	for (pair<int, int> i : store)
	{
		...
		this->grid[new_room_pair.first][new_room_pair.second] = new_room;
	}
...
void world::put_pit(pair<int, int> pit)
{
	...
	this->grid[pit.first][pit.second] = pit_room;
}
void world::put_gold(pair<int, int> gold)
{
	...
	this->grid[gold.first][gold.second] = gold_room;
}
void world::put_wumpus(pair<int, int> wumpus)
{
	...
	this->grid[wumpus.first][wumpus.second] = wumpus_room;
}
void world::kill_wumpus_at(pair<int, int> room_id)
{
